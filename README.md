...
- Reads from local CSVs. On a real programme this would sit alongside the
  client's existing scheduling and cost tools (Primavera P6, MS Project, an
  ERP cost module), exporting the fields it needs, not replacing them as the
  system of record.
- Risk probability/impact scores and change approval decisions are taken as
  given inputs, not generated by the tool. Deciding what a risk scores or
  whether a change gets approved stays a PMO judgment call; this reports
